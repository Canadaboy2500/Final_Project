...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.lasers >= 1 and pygame.time.get_ticks() - self.laser_time > self.LASERTIME:
            self.lasers -= 1
            if self.lasers > 1:
                player_laser = o.Laser(self, 6000, 'player', self.dmg/4)
    ...
    def shoot(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            self.last_shot = now
            if self.power == 0:
                bullet = o.Bullet(self.rect.centerx, self.rect.top, -10, self.dmg, 'heavy_bullets')
                all_sprites.add(bullet)
                bullets.add(bullet)
                shoot_sound.play()
            if self.power >= 1:
                bullet1 = o.Bullet(self.rect.left, self.rect.centery, -10, self.dmg, 'heavy_bullets')
                bullet2 = o.Bullet(self.rect.right, self.rect.centery, -10, self.dmg, 'heavy_bullets')
