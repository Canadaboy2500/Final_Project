...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.dead == False:
            ...
            if keystate[pygame.K_SPACE]:
                if self.lasers  >= 1:
                    player_laser.on = True
                else:
                    self.shoot()
                    player_laser.on = False
            else:
                player_laser.on = False
    ...
    def shoot(self):
        ...
        if now - self.last_shot > self.shoot_delay:
            self.last_shot = now
            if self.power == 0:
                bullet = o.Bullet(self.rect.centerx, self.rect.top)
                all_sprites.add(bullet)
                bullets.add(bullet)
                shoot_sound.play()
            if self.power >= 1:
                bullet1 = o.Bullet(self.rect.left, self.rect.centery)
                bullet2 = o.Bullet(self.rect.right, self.rect.centery)
